...
class GoogleAnalyticsStats
{
	...
	function GoogleAnalyticsStats()
	{
            ...
            $this->client->setClientId('920779528356-9p3ollu248gk117f2prle6vt0osh781r.apps.googleusercontent.com');
            $this->client->setClientSecret('vsedOYCCFDcgYA2UoFsFttE3');
            $this->client->setRedirectUri('urn:ietf:wg:oauth:2.0:oob');
			
            $this->client->setScopes(array(GOOGLE_MV-ANALYTICS_SCOPE));
	...
	function checkLogin()
	{
            $topPost_google_authtoken  = get_option('topPost_google_authtoken');

            if (!empty($topPost_google_authtoken))
            {
                    $this->client->setAccessToken($topPost_google_authtoken);
            }
            else
            {
                $authCode = get_option('topPost_google_token');
                ...
                {
                    print '(cas:72) Google top posts plugin was unable to authenticate you with
                            ...
                            <a href="' . admin_url('/options-general.php?page=topPost_reset').'"> Try Deauthorizing &amp; Resetting Google Analyticator.</a>
                ...
                if($accessToken)
                {
                    ...
                    update_option('topPost_google_authtoken', $accessToken);
                }
                else
                {
                    return false;
	...
	{
            update_option('topPost_google_token', '');
            update_option('topPost_google_authtoken', '');
	}

	function getSingleProfile()
	{
            $webproperty_id = get_option('topPost_uid');
